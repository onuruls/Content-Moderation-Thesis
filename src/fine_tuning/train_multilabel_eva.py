...
# EVA tabanlı timm modeli (selected_tags YOK)
...
# Checkpoint & Log
OUT_DIR = "../last_animetimm_multilabel"
...
PATIENCE = 2  # üst üste bu kadar epoch gelişme yoksa (head-only fazında) UNFREEZE tetikle
MIN_EPOCHS = 3  # çok erken kesmeyi önle (head-only için)
...
UNFREEZE_MIN_EPOCH = 4  # en az şu epoch görülsün; plateau + bu koşul → unfreeze
UNFREEZE_PATIENCE = 2  # unfreeze fazında plateau sonrası durdurma sabrı
UNFREEZE_PLATEAU_DELTA = 0.003  # unfreeze fazında daha sıkı/gevşek eşik
...
def set_trainable_unfreeze_last_blocks(model: EVAMultiLabelModel, last_blocks: int):
    ...
    if hasattr(model.backbone, "blocks") and isinstance(model.backbone.blocks, (list, nn.ModuleList, nn.Sequential)):
        ...
        # fallback: son çocuk modülleri aç
...
def pick_thresholds(val_loader, model):
    ...
    L = torch.cat(Ls).numpy();
    Y = torch.cat(Ys).numpy();
    M = torch.cat(Ms).numpy()
...
def train():
    ...
    if resumed_control is not None:
        control.update(resumed_control)
        append_log(f"[control] resume state: {control}")

        # Eğer resume "unfreeze" fazında ise param grupları doğru mu?
    ...
    # === DEĞİŞİKLİK: Eğer epoch 29'dan büyüksek, direkt val/test yap ===
    if epoch > STOP_AT_EPOCH:
        append_log(f">> Epoch {epoch} > {STOP_AT_EPOCH}, sadece val/test yapılacak")
        # Doğrudan validation ve test yap
        val_thresholds = pick_thresholds(val_loader, model)
        val_per, val_agg = evaluate(val_loader, model, val_thresholds)

        # === DÜZELTME: Test için DOĞRU threshold'ları kullan ===
        # control state'den best threshold'ları al
        if control["best_val_thresholds"] is not None:
            best_th_np = np.array([control["best_val_thresholds"][c] for c in CLASS_NAMES], dtype=np.float32)
            append_log(f"[test] Best val thresholds kullanılıyor: {control['best_val_thresholds']}")
        else:
            # Fallback: yeni hesaplanan threshold'ları kullan
            best_th_np = val_thresholds
            append_log(f"[test] Yeni val thresholds kullanılıyor: {val_thresholds}")

        test_per, test_agg = evaluate(test_loader, model, best_th_np)

        # Sonuçları kaydet
        with open(os.path.join(OUT_DIR, "final_val_test_results.json"), "w", encoding="utf-8") as f:
            json.dump({
                "val_results": val_per,
                "val_aggregate": val_agg,
                "test_results": test_per,
                "test_aggregate": test_agg,
                "epoch": epoch,
                "thresholds_used": control["best_val_thresholds"] if control[
                                                                         "best_val_thresholds"] is not None else val_thresholds.tolist(),
                "note": f"Direct evaluation at epoch {epoch}"
            }, f, indent=2)

        append_log(">> Sadece val/test tamamlandı!")
        return
    ...
    try:
        while True:
            ...
            # best güncelle ve TEST'i sadece best olduğunda çalıştır
            improved = False
            if macro > control["best_macro_f1"] + 1e-12:
                ...
                # Hız için testi sadece best güncellenince koş
                best_th_np = np.array([control["best_val_thresholds"][c] for c in CLASS_NAMES], dtype=np.float32)
                test_per, test_agg = evaluate(test_loader, model, best_th_np)
                with open(os.path.join(OUT_DIR, "best_test.json"), "w", encoding="utf-8") as f:
                    json.dump({"per_class": test_per, "aggregate": test_agg}, f, indent=2)
                append_log(
                    f"[test] (best-so-far) macroF1={test_agg['macro_f1']:.4f} microF1={test_agg['micro_f1']:.4f}")
            ...
            # === DEĞİŞİKLİK: Epoch 29'da dur =========
            if epoch >= STOP_AT_EPOCH:
                append_log(f"[auto] Epoch {STOP_AT_EPOCH}'a ulaşıldı. Eğitim durduruluyor.")
                break
            ...
            if (control["phase"] == "head"
                ...
                # Optimizer & scheduler'ı UNFREEZE fazı için yeniden kur
                optimizer = build_optimizer_unfreeze(model)
                scheduler = build_scheduler(optimizer, iters_per_epoch)
            ...
            # ---- OTO-DURDURMA (fazlara göre) ----
            if control["phase"] == "head":
                stop_regress = STOP_ON_REGRESSION and (macro < control["best_macro_f1"] - REGRESSION_DELTA) and (
                            control["total_epochs"] >= MIN_EPOCHS)
                stop_plateau = False  # head fazında plateau → UNFREEZE yapıldı, direkt stop etme
            else:
                # unfreeze fazında plateau ve/veya regression'da dur
                stop_regress = STOP_ON_REGRESSION and (macro < control["best_macro_f1"] - REGRESSION_DELTA) and (
                            control["total_epochs"] >= MIN_EPOCHS)
            ...
            if stop_regress:
                append_log(
                    f"[auto] stop (regression): val_macro={macro:.4f} < best-{REGRESSION_DELTA}={control['best_macro_f1'] - REGRESSION_DELTA:.4f}")
                break
            if stop_plateau:
                append_log(
                    f"[auto] stop (plateau): {control['plateau']} >= {(UNFREEZE_PATIENCE if control['phase'] == 'unfreeze' else PATIENCE)}")
                break
    ...
    torch.save({
        ...
        "config": {
            ...
            "unfreeze_blocks": UNFREEZE_LAST_BLOCKS, "stop_at_epoch": STOP_AT_EPOCH
        }
    }, final_path)
    with open(os.path.join(OUT_DIR, "thresholds.json"), "w", encoding="utf-8") as f:
        json.dump(
            {c: float(t) for c, t in zip(CLASS_NAMES, (best_th if best_th is not None else [0.5] * len(CLASS_NAMES)))},
            f, indent=2)
