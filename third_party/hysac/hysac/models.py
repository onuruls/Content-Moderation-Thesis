#! src/models/HySAC/hysac/models.py
import math
import json
import torch
...
def _strict_align_and_fill(model: torch.nn.Module, state: dict) -> dict:
    """
    Checkpoint anahtarlarını model ile hizalar:
      - .base_layer. ekini kaldırır
      - Modelde olup ckpt'de olmayan anahtarları (ör. bias) modelin mevcut değerleriyle doldurur
      - Ckpt'de olup modelde olmayan anahtarları (tamamen yabancı) atar
    Sonuç: load_state_dict(strict=True) sorunsuz çalışır.
    """
    state = {k.replace(".base_layer.", "."): v for k, v in state.items()}
...
class HySAC(CLIPBaseline):
    ...
    def from_pretrained(cls, repo_id: str = "aimagelab/HySAC", device="cpu"):
        ...
        state = _strict_align_and_fill(model, state)
